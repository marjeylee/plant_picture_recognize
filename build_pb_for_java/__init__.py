...
class ClassifyModel:
    ...
    def __init__(self, img_batch_size):
        ...
        self.keep_prob20 = tf.constant(value=0.95, dtype=tf.float32, name='keep_prob20')
        ...
        self.keep_prob22 = tf.constant(value=0.95, dtype=tf.float32, name='keep_prob22')
    ...
    def get_pb(self):
        ...
        with tf.Session(
                config=tf.ConfigProto(gpu_options=(tf.GPUOptions(per_process_gpu_memory_fraction=0.7)))) as sess:
            saver.restore(sess, "./Model/model")
            layer23 = sess.graph.get_tensor_by_name('full_connection_layer23/full_connection_layer23_fc:0')
            constant_graph = tf.graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                          ['probability',
                                                                           'full_connection_layer23/full_connection_layer23_fc'])
